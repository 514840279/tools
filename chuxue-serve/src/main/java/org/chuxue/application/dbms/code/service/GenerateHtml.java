...
public class GenerateHtml {
	...
	public static void generateVue3(SysDbmsGenerateCodeInfo sysDbmsGenerateCodeInfo, SysDbmsTabsTableInfo tabsInfo, List<SysDbmsTabsColsInfo> colsInfos, String username, String pathString) {
		...
		String subPath = pathString.substring(pathString.indexOf("views/") + 6);
		String arrpaString = "";
		if (StringUtils.isBlank(subPath)) {
			String[] aStrings = subPath.split("/");
		...
		stringBuilder.append("import Table from \"" + arrpaString + "../components/table/Table.vue\";\r\n");
		stringBuilder.append("import { Column } from \"" + arrpaString + "../interface/Table\";\r\n");
		stringBuilder.append("");
		...
		for (SysDbmsTabsColsInfo sysDbmsTabsColsInfo : colsInfos) {
			...
			} else if ("uuid".equals(colsName) || "create_time".equals(colsName) || "create_user".equals(colsName) || "update_time".equals(colsName) || "update_user".equals(colsName)) {
	...
	public static void generateRouter(SysDbmsGenerateCodeInfo sysDbmsGenerateCodeInfo, SysDbmsTabsTableInfo tabsInfo, List<SysDbmsTabsColsInfo> colsInfos, String username, String pathString) {
		...
		stringBuilder.append("    component: () => import('../views/" + subPath + "/Index.vue'),\r\n");
		
		// 文件写入
		String fineName = pathString + "/" + sysDbmsGenerateCodeInfo.getClassName().toLowerCase() + "/addRouter.json";
		TxtFilesWriter.writeToFile(stringBuilder.toString(), fineName);
