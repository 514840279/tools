package org.chuxue.application.imp;
...
//		Connection conn = MysqlConnUtils.getConnection();
		...
		while (map.hasNext()) {
			...
			while (columns.hasNext()) {
				...
				data = data.replace("\\", "").replace("null", "").replace("NULL", "").replace("&", "").replace("#", "")
				...
				if (e.getErrorCode() == 1704) {
					...
					while (columns.hasNext()) {
						...
						if (columnName == idName) {
							idData = row.get(columnName) == null ? UUID.randomUUID().toString()
					...
					while (columns.hasNext()) {
						...
						data = data.replace("\\", "").replace("null", "").replace("NULL", "").replace("&", "")
					...
					while (columns.hasNext()) {
						...
						if (row.get(columnName) != null && row.get(columnName).toString().length() > 1000) {
							...
							while (data.length() > 0) {
								...
								updateBuilder.append(" set " + columnName + "=" + columnName + "||'"
								...
								try {
									...
									if (ex.getErrorCode() == 1704) {
										/* 修改原字段名name为name_tmp */
										statement.execute("alter table " + tableName + " rename column " + columnName
												+ " to name_tmp");
										...
										/* 方式二: */
										statement.execute(
												"update " + tableName + " set " + columnName + " =trim(name_tmp)");
